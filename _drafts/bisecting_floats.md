...
{% highlight julia %}
julia> bisect1(x -> 1e4*sin(x), 2.0, 4.0)
# loops forever...
julia> bisect2(x -> 1e4*sin(x), 2.0, 4.0)
...
Over floating point doubles, the first algorithm doesn't terminate because there is no floating point number <span class='mathquill-embedded-latex'>x</span> between 2.0 and 4.0 such that <span class='mathquill-embedded-latex'>10^4\sin(x) &lt; 10^{-12}</span>, and the second algorithm does terminate because for any finite floating point number <span class='mathquill-embedded-latex'>x_1</span> (except the largest finite float), there exists a strictly larger floating point number <span class='mathquill-embedded-latex'>x_2</span> such that <span class='mathquill-embedded-latex'>(x_1 + x_2)/2 = x_1</span> or <span class='mathquill-embedded-latex'>(x_1 + x_2)/2 = x_2</span>.

Both of these results are due to the granularity of floating point numbers. There is a finite gap between any floating point number and the next largest floating point number. The size of the gap depends (proportionally) on the size of the number. In Julia, you can find the size of the gap using `eps`:
...
Neither of these numbers is within <span class='mathquill-embedded-latex'>10^{-16}</span> of 0.0, which is why `bisect1` fails to terminate. On the other hand, `bisect2` managed to find exactly these numbers as lower and upper bounds of a root of `sin`. It didn't need any explicit tolerances at all.
...
Choosing a different arbitrary tolerance in a general purpose floating point algorithm is impolite. Absolute tolerances like the `1e-12` in `bisect1` are inappropriate in general purpose algorithms because floating point numbers don't come with units attached, so an algorithm with an absolute tolerance will behave differently depending on whether your user measures lengths in millimeters or meters. Relative tolerances fall down when the answer is supposed to be 0. The spacing between floating point numbers cleanly elides these two limits, being relative for finite numbers, and finite for 0.
...
Earlier, I said "each step of bisection should halve the number of floats contained in the interval," but as written, `bisect_step` doesn't actually do this. The problem is that floats aren't evenly distributed. They are much denser near 0 than far from it. This means that bisecting toward zero using the arithmetic mean eliminates fewer than half of the floats in the interval at each step.

The algorithm can be improved by choosing a midpoint that bisects the set of floats instead of the length of a real interval. Here is such a function:
...
It works by taking the mean directly on the binary representation of floating point numbers, instead of on their real number values. The operation is approximately the geometric mean. I won't comment here on the minor complications that arise from negative inputs.
